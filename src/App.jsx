...
async function fetchAll(userId) {
  const [txRes, goalsRes, splitRes, catsRes] = await Promise.all([
    supabase.from("transactions").select("*").eq("user_id", userId).order("id"),
    supabase.from("goals").select("*").eq("user_id", userId),
    supabase.from("split_template").select("*").eq("user_id", userId).order("id"),
    supabase.from("categories").select("*").eq("user_id", userId),
  ]);
  return {
    tx: (txRes.data||[]).map(r=>({id:r.id,type:r.type,category:r.category,amount:Number(r.amount),month:r.month,year:r.year,note:r.note||""})),
    goals: (goalsRes.data||[]).map(r=>({id:r.id,name:r.name,target:Number(r.target),saved:Number(r.saved),color:r.color,deadline:r.deadline||""})),
    split: (splitRes.data||[]).map(r=>({id:r.id,label:r.label,category:r.category,pct:Number(r.pct),amount:r.amount||"",type:r.type,goalId:r.goal_id})),
    expCats: (catsRes.data||[]).filter(c=>c.list_type==="expense").map(c=>c.name),
    incCats: (catsRes.data||[]).filter(c=>c.list_type==="income").map(c=>c.name),
  };
}

async function seedDefaults(userId) {
  await supabase.from("transactions").insert(DEMO_TX.map(t=>({...t,user_id:userId})));
  await supabase.from("goals").insert(DEMO_GOALS.map(g=>({...g,user_id:userId})));
  await supabase.from("split_template").insert(DEFAULT_SPLIT.map(s=>({...s,goal_id:s.goalId,user_id:userId})));
  await supabase.from("categories").insert([
    ...DEFAULT_EXP_CATS.map(name=>({list_type:"expense",name,user_id:userId})),
    ...DEFAULT_INC_CATS.map(name=>({list_type:"income",name,user_id:userId})),
  ]);
}

async function upsertTx(userId, t) {
  await supabase.from("transactions").upsert({id:t.id,user_id:userId,type:t.type,category:t.category,amount:t.amount,month:t.month,year:t.year,note:t.note||""});
}
async function deleteTx(id) { await supabase.from("transactions").delete().eq("id", id); }

async function upsertGoal(userId, g) {
  await supabase.from("goals").upsert({id:g.id,user_id:userId,name:g.name,target:g.target,saved:g.saved,color:g.color,deadline:g.deadline||""});
}
async function deleteGoalRow(id) { await supabase.from("goals").delete().eq("id", id); }

async function upsertSplitRow(userId, r) {
  await supabase.from("split_template").upsert({id:r.id,user_id:userId,label:r.label,category:r.category,pct:r.pct,amount:String(r.amount??""),type:r.type,goal_id:r.goalId});
}
async function deleteSplitRow(id) { await supabase.from("split_template").delete().eq("id", id); }

async function addCategoryRow(userId, listType, name) {
  await supabase.from("categories").insert({user_id:userId,list_type:listType,name});
}
async function deleteCategoryRow(userId, listType, name) {
  await supabase.from("categories").delete().eq("user_id",userId).eq("list_type",listType).eq("name",name);
}
async function resetCategoriesRow(userId, listType, names) {
  await supabase.from("categories").delete().eq("user_id",userId).eq("list_type",listType);
  await supabase.from("categories").insert(names.map(name=>({user_id:userId,list_type:listType,name})));
}

// ══════════════════════════════════════════════════════════════════
function BudgetApp({ userId, onSignOut }) {
  // ── Persisted state (loaded from Supabase) ──
  const [loaded,  setLoaded]   = useState(false);
  const [tx,      setTxRaw]    = useState([]);
  const [goals,   setGoalsRaw] = useState([]);
  const [split,   setSplitRaw] = useState([]);
  const [expCats, setExpCatsRaw] = useState(DEFAULT_EXP_CATS);
  const [incCats, setIncCatsRaw] = useState(DEFAULT_INC_CATS);
  const [syncing, setSyncing] = useState(false);

  useEffect(() => {
    let cancelled = false;
    (async () => {
      let d = await fetchAll(userId);
      const isEmpty = !d.tx.length && !d.goals.length && !d.split.length && !d.expCats.length;
      if (isEmpty) {
        await seedDefaults(userId);
        d = await fetchAll(userId);
      }
      if (cancelled) return;
      setTxRaw(d.tx);
      setGoalsRaw(d.goals);
      setSplitRaw(d.split);
      setExpCatsRaw(d.expCats.length ? d.expCats : DEFAULT_EXP_CATS);
      setIncCatsRaw(d.incCats.length ? d.incCats : DEFAULT_INC_CATS);
      setLoaded(true);
    })();
    return () => { cancelled = true; };
  }, [userId]);

  // setTx: optimistic local update + sync diffed rows to Supabase
  const setTx = useCallback((updater) => {
    setTxRaw(prev => {
      const next = typeof updater === "function" ? updater(prev) : updater;
      const prevIds = new Set(prev.map(t=>t.id));
      const nextIds = new Set(next.map(t=>t.id));
      next.forEach(t => { if (!prevIds.has(t.id) || prev.find(p=>p.id===t.id) !== t) upsertTx(userId, t); });
      prev.forEach(t => { if (!nextIds.has(t.id)) deleteTx(t.id); });
      return next;
    });
  }, [userId]);
  ...
  const setGoals = useCallback((updater) => {
    setGoalsRaw(prev => {
      ...
      next.forEach(g => { if (!prevIds.has(g.id) || prev.find(p=>p.id===g.id) !== g) upsertGoal(userId, g); });
      prev.forEach(g => { if (!nextIds.has(g.id)) deleteGoalRow(g.id); });
      return next;
    });
  }, [userId]);

  const setSplit = useCallback((updater) => {
    setSplitRaw(prev => {
      const next = typeof updater === "function" ? updater(prev) : updater;
      const prevIds = new Set(prev.map(r=>r.id));
      const nextIds = new Set(next.map(r=>r.id));
      next.forEach(r => { if (!prevIds.has(r.id) || prev.find(p=>p.id===r.id) !== r) upsertSplitRow(userId, r); });
      prev.forEach(r => { if (!nextIds.has(r.id)) deleteSplitRow(r.id); });
      return next;
    });
  }, [userId]);

  const setExpCats = useCallback((updater) => {
    setExpCatsRaw(prev => {
      const next = typeof updater === "function" ? updater(prev) : updater;
      const added = next.filter(c=>!prev.includes(c));
      const removed = prev.filter(c=>!next.includes(c));
      if (added.length===1 && removed.length===0 && next.length===prev.length+1) addCategoryRow(userId,"expense",added[0]);
      else if (removed.length===1 && added.length===0) deleteCategoryRow(userId,"expense",removed[0]);
      else resetCategoriesRow(userId,"expense",next);
      return next;
    });
  }, [userId]);

  const setIncCats = useCallback((updater) => {
    setIncCatsRaw(prev => {
      const next = typeof updater === "function" ? updater(prev) : updater;
      const added = next.filter(c=>!prev.includes(c));
      const removed = prev.filter(c=>!next.includes(c));
      if (added.length===1 && removed.length===0 && next.length===prev.length+1) addCategoryRow(userId,"income",added[0]);
      else if (removed.length===1 && added.length===0) deleteCategoryRow(userId,"income",removed[0]);
      else resetCategoriesRow(userId,"income",next);
      return next;
    });
  }, [userId]);
  ...
  if (!loaded) {
    return (
      <div style={{background:P.bg,minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Inter',system-ui,sans-serif",color:P.muted,fontSize:14}}>
        Загружаю данные…
      </div>
    );
  }
      ...
      <div style={{padding:"22px 20px 0",borderBottom:`1px solid ${P.border}`,background:P.bg}}>
        <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between"}}>
          <div>
            ...
            <div style={{fontSize:12,color:P.muted,marginBottom:14}}>Синхронизировано · доступно с любого устройства</div>
          </div>
          <button onClick={onSignOut} style={{...btn(P.muted,true),padding:"5px 12px",fontSize:12,marginTop:2}}>Выйти</button>
        </div>
...
// ══════════════════════════════════════════════════════════════════
// App — handles auth session, shows AuthScreen or BudgetApp
export default function App() {
  ...
  return <BudgetApp userId={session.user.id} onSignOut={()=>supabase.auth.signOut()} />;
}
